...
module FullNameSplitter
  ...
  PREFIXES = %w(de da du del dei vda. dello della degli delle van von der den heer ten ter vande vanden vander voor ver aan mc).freeze
  ...
  class Splitter
    ...
    def split!
      ...
      while @unit = @units.shift do
        if prefix? or with_apostrophe? or (first_name? and last_unit? and not initial?) or last_unit?
